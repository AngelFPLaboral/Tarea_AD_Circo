...
public class GestionPersona {
	...
	public void registrarPersona() {
		...
		try {
			...
			// son usadas en todos los procesos son las que podemos encontrar en las clases:
			// PERSONA, ARTISTA, COORDINACION
			String nombre = null;
			String email = null;
				...
				try {
					...
					case 1:
						...
						do {
							...
							if (verificarCancelar(respuesta))
								return;
							...
							case "S":
								...
								do {
									...
									if (verificarCancelar(fecha))
										return;

									try {
										// DateTimeFormatter aprendio en Interfaces
										// Soy conciente que existe un LocalDate.parse(), pero creo que ese funiona con
										// el yyyy-MM-dd, que para mi es un sistema/formado incomodo (opinion propia),
										// si hay algun apunte o cambio puedo editarlo
										DateTimeFormatter formato = DateTimeFormatter.ofPattern("dd-MM-yyyy");
										fechaSenior = LocalDate.parse(fecha, formato);

										if (fechaSenior.isAfter(LocalDate.now())) {
											System.out.println(
													"ERROR: La fecha no puede ser superior a la fecha actual.");
											fechaValida = false;
										} else {
											fechaValida = true;
					...
					default:
						System.out.println("ERROR: Opcion invalida. Debe ser 1 o 2.");
						break;
					}
			...
			do {
				// Yo esto me lo imagine como en una app que el nombre de usuario y la de la
				// cuenta pueden ser diferentes
				System.out.print("Nombre de usuario (credenciales/cuenta): ");
				nombreUsuario = sc.nextLine().trim();

				if (verificarCancelar(nombreUsuario))
					return;
			...
			do {
				...
				if (verificarCancelar(contraseña))
					return;
			...
			long nuevoId = FicherosUtils.incrementarId(); // Aqui ocurre el una escala de tipos numéricos primitivos,
															// donde el porpio programa pasa el int a long cuando entra
															// a Persona
	...
	private String seleccionarNacionalidad() {
		...
		do {
			...
			if (idPais.equalsIgnoreCase("CANCELAR")) {
	...
	/**
	 * Metodo: sirve para seleccionar las especialidades que quiere introducirle al admin al usuario
	 * @return
	 */
	private List<Especialidad> seleccionarEspecialidades() {
		...
		do {
			...
			if (especialidades.isEmpty()) {
				System.out.println("ERROR: No se seleccionaron especialidades validas.");
				System.out.println("Por favor, intente de nuevo.");
			} else {
